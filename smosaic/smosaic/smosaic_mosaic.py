...
from smosaic.smosaic_new_methods import NEW_METHODS, COMPUTED_BANDS, compose_new_method
...
def mosaic(name, data_dir, stac_url, collection, output_dir, start_year, start_month, start_day, mosaic_method, grid_crop=False, bands=None, reference_date=None, duration_days=None, end_year=None, end_month=None, end_day=None, duration_months=None, geom=None, grid=None, tile_id=None, bbox=None, profile=None, projection_output=4326, k=1, banda_ref=None):
    ...
    # Expand derived bands (e.g. NBR) to real STAC bands before querying
    if bands:
        query_bands = []
        for b in bands:
            if b in COMPUTED_BANDS:
                for rb in COMPUTED_BANDS[b]:
                    if rb not in query_bands:
                        query_bands.append(rb)
            else:
                if b not in query_bands:
                    query_bands.append(b)
    else:
        query_bands = bands
    ...
    if not os.path.exists(collection_dir):
        ...
        scene_dirs = [
            s for s in os.listdir(collection_dir)
            if os.path.isdir(os.path.join(collection_dir, s))
        ]
        bands_present = scene_dirs and all(
            any(os.path.isdir(os.path.join(collection_dir, s, b)) for s in scene_dirs)
...
def process_period(period, mosaic_method, data_dir, collection_name, bands, bbox, output_dir, duration_days, duration_months, name, geom, reference_date, projection_output, grid, tile_id, k=1, banda_ref=None):
    ...
    # ── Phase 1: build sorted_data for ALL bands ──────────────────────────────
    ...
    # Derived bands (e.g. NBR) are expanded to the real bands stored on disk.
    ...
    for i, current_band in enumerate(phase1_bands):
        ...
        for path in scenes:
            ...
            for file in filtered_files:
                ...
                if reference_date:
                    ...
                    pixel_count = count_pixels(os.path.join(coll_data_dir, path, cloud, file), cloud_dict[collection_name]['non_cloud_values'], geom)
                    ...
                    band_list.append(dict(band=current_band, date=date.strftime("%Y%m%d"), distance_days=distance_days, clean_percentage=float(pixel_count['count'] / pixel_count['total']), scene=path, file=''))
                else:
                    pixel_count = count_pixels(os.path.join(coll_data_dir, path, cloud, file), cloud_dict[collection_name]['non_cloud_values'], geom)
                    ...
                    band_list.append(dict(band=current_band, date=date.strftime("%Y%m%d"), clean_percentage=float(pixel_count['count'] / pixel_count['total']), scene=path, file=''))

        files_list = []
        for path in scenes:
            for band in [current_band, cloud]:
                band_dir = os.path.join(coll_data_dir, path, band)
                ...
                for file in filtered_files:
                    files_list.append(dict(file=os.path.join(band_dir, file)))
        ...
        if mosaic_method == 'lcf' or mosaic_method in NEW_METHODS:
            sorted_band = sorted(band_list, key=lambda x: x['clean_percentage'], reverse=True)
            sorted_cloud = sorted(cloud_list, key=lambda x: x['clean_percentage'], reverse=True)
        elif mosaic_method == 'chrono':
            sorted_band = sorted(band_list, key=lambda x: x['date'])
            sorted_cloud = sorted(cloud_list, key=lambda x: x['date'])
        elif mosaic_method == 'ctd':
            sorted_band = sorted(band_list, key=lambda x: x['distance_days'])
            sorted_cloud = sorted(cloud_list, key=lambda x: x['distance_days'])
        else:
            sorted_band = sorted(band_list, key=lambda x: x['date'])
            sorted_cloud = sorted(cloud_list, key=lambda x: x['date'])

        reproject_data = reproject_tifs(sorted_data=sorted_band, cloud_sorted_data=sorted_cloud, data_dir=data_dir, projection_output=projection_output)
        all_sorted_data[current_band] = reproject_data['reprojected_images']
        if i == 0:
            cloud_sorted_data = reproject_data['reprojected_cloud_images']

    # ── Phase 2: Compose ──────────────────────────────────────────────────────

    if mosaic_method in NEW_METHODS:
        composition_results = compose_new_method(
            all_sorted_data=all_sorted_data,
        ...
        for i, current_band in enumerate(bands):
            if i == 0:
                res = merge_scene_provenance_cloud(all_sorted_data[current_band], cloud_sorted_data, scenes, collection_name, current_band, data_dir, start_date, end_date)
            else:
                res = merge_scene(all_sorted_data[current_band], cloud_sorted_data, scenes, collection_name, current_band, data_dir, start_date, end_date)
            composition_results[current_band] = res

    # ── Phase 3: Post-process — same for both paths ───────────────────────────
    ...
    for i, current_band in enumerate(bands):
        ...
        ordered_lists = composition_results[current_band]
        ...
        datasets = [rasterio.open(f) for f in ordered_lists['merge_files']]
        extents = get_dataset_extents(datasets)

        merge_tifs(tif_files=ordered_lists['merge_files'], output_path=output_file, band=current_band, path_row=name, extent=extents)

        if i == 0 and 'provenance_merge_files' in ordered_lists:
            provenance_output_file = os.path.join(output_dir, f"provenance_raw-{file_name}.tif")
            merge_tifs(tif_files=ordered_lists['provenance_merge_files'], output_path=provenance_output_file, band=current_band, path_row=name, extent=extents)

        if i == 0 and 'cloud_merge_files' in ordered_lists:
            cloud_data_output_file = os.path.join(output_dir, f"cloud_data_raw-{file_name}.tif")
            merge_tifs(tif_files=ordered_lists['cloud_merge_files'], output_path=cloud_data_output_file, band=cloud, path_row=name, extent=extents)
